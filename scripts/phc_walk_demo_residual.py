"""IsaacGym interactive demo: trained residual + AMP + v_cmd walking.

Loads the Stage 2 trained residual policy (ResAMPVCmd) on top of frozen phc_3.
Unlike scripts/phc_walk_demo.py (zero-training, multi-clip + retime + a-1
hard switch), this demo just lets the trained network handle v_cmd:
  - The task class still does multi-clip switching during *training*, but at
    inference the policy already learned to span [V_CMD_MIN, V_CMD_MAX]
    continuously, so we keep v_cmd evolution but skip the demo-level clip
    switching monkey-patches that phc_walk_demo.py needed.
  - The keyboard / panel UX (↑↓ to nudge, 1..9 to snap, R/P/Q controls,
    optional --record_seconds) is unchanged.

Differences from scripts/phc_walk_demo.py:
- Loads the trained ResAMPVCmd checkpoint via run.py's --test --epoch -1
  path (resolves to output/HumanoidIm/ResAMPVCmd/Humanoid_<latest>.pth).
- Skips the demo-level clip-switch monkey-patches (the trained policy
  is supposed to interpolate continuously across v_cmd ∈ [0.76, 1.23],
  so we don't need to swap reference clips at runtime).
- Skips the per-step retime injection (_motion_start_times_offset
  accumulation in pre_physics_step is kept but the task class itself
  drives it during training; at inference v_cmd alone is the input).

Run:
  conda activate phc
  python scripts/phc_walk_demo_residual.py
  # optional 30s recording:
  python scripts/phc_walk_demo_residual.py --record_seconds 30 \\
                                           --out_name res_amp_vcmd_smoke
...
TASK_NAME = "HumanoidImResAMPVCmd"               # argparse boot path uses this
...
def _install_render_hook():
    ...
    def patched_render(self, sync_frame_time=False):
        ...
        _handle_events(self)
        # NOTE: _maybe_switch_clip is intentionally NOT called here. In the
        # zero-training v3 demo it picked among 3 natural-speed clips and
        # retimed; for the trained-residual demo, HumanoidImResAMPVCmd handles
        # both clip selection and per-step retime inside the task class
        # (see _select_clip_idx / _compute_observations / _ramp_v_cmd). Adding
        # demo-level clip switching here would double-fire and stamp on the
        # task's own _global_offset accounting.
        # Pull active_clip from the task for the panel display.
        if hasattr(self, "_active_clip_idx"):
            ac = int(self._active_clip_idx[0].item())
            _STATE["active_clip"] = ac
            if 0 <= ac < len(V_NATURAL):
                _STATE["v_natural"] = V_NATURAL[ac]
...
def _install_pre_physics_patch():
    """Patch Humanoid.pre_physics_step.

    For the trained-residual demo, the heavy lifting (clip selection,
    per-step retime) is done by HumanoidImResAMPVCmd itself. The demo
    just needs to:
      1. Push the user's keyboard/panel v_cmd into task._v_cmd_target.
      2. Let the task's own _ramp_v_cmd evolve _v_cmd_ramped per its
         training-time policy (so the inference distribution matches).
      3. Read back v_cmd_ramped into _STATE for the panel to display.
    """
    from phc.env.tasks.humanoid import Humanoid
    ...
    def patched_pre(self, actions):
        if not _STATE["paused"]:
            # Push user-set v_cmd target into the task; the task's own
            # _ramp_v_cmd handles the smooth evolution and clip switching.
            if hasattr(self, "_v_cmd_target"):
                self._v_cmd_target.fill_(_STATE["v_cmd_target"])
            # Mirror the task's current v_cmd_ramped back into _STATE for
            # the panel (the task is the source of truth at inference).
            if hasattr(self, "_v_cmd_ramped"):
                _STATE["v_cmd_ramped"] = float(self._v_cmd_ramped[0].item())
...
def main():
    ...
    # Boot via run.py (argparse) — HumanoidImResAMPVCmd is registered there.
    # --test --epoch -1 picks the latest Humanoid_*.pth in
    # output/HumanoidIm/ResAMPVCmd/.
    sys.argv = [
        "phc/run.py",
        "--task", TASK_NAME,
        "--cfg_env", f"phc/data/cfg/env/{ENV_CFG_NAME}.yaml",
        "--cfg_train", f"phc/data/cfg/learning/{LEARNING}.yaml",
