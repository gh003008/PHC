...
OUT_DIR="analysis/plots/phc_pain_v15_trials/videos/solo"
...
run_play() {
  local exp_name="$1"
  PHC_SHOW_TRAJ=0 \
  PHC_VIS_BASE_BODY_COLOR="${PHC_VIS_BASE_BODY_COLOR:-0.05,0.35,0.95}" \
  PHC_VIS_RIGHT_KNEE_PAIN="${PHC_VIS_RIGHT_KNEE_PAIN:-1}" \
  PHC_VIS_RIGHT_KNEE_PAIN_SOURCE="${PHC_VIS_RIGHT_KNEE_PAIN_SOURCE:-load}" \
  PHC_VIS_RIGHT_KNEE_PAIN_SCALE="${PHC_VIS_RIGHT_KNEE_PAIN_SCALE:-1.5}" \
...
record_trial() {
  ...
  echo "=== Solo screen recording ${label}: ${exp_name} ==="
