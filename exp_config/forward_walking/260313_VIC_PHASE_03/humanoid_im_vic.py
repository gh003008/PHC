...
class HumanoidImVIC(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # VIC (Variable Impedance Control) Initialization
        self._vic_enabled = cfg["env"].get("vic_enabled", False)
        self._vic_curriculum_stage = cfg["env"].get("vic_curriculum_stage", 1)
        ...
        # VIC: Build CCF group map after super().__init__() since _dof_size is now available
        if self._vic_enabled and self._vic_ccf_num_groups > 0:
    ...
    def get_action_size(self):
        if self._vic_enabled:
            # VIC: use grouped CCF if configured, otherwise per-DOF CCF
            ccf_dims = self._vic_ccf_num_groups if self._vic_ccf_num_groups > 0 else self._num_actions
            return self._num_actions + ccf_dims
    ...
    def _compute_bio_ccf_reward(self):
        ...
        Stance leg (foot on ground) → reward ankle CCF > 0 (stiff for weight bearing)
        Swing leg (foot in air) → reward ankle CCF < 0 (compliant for leg swing)

        Uses contact_bodies: ["R_Ankle", "L_Ankle", "R_Toe", "L_Toe"]
        CCF groups: G2=L_Ankle+Toe, G5=R_Ankle+Toe
        """
        ...
        # Get current CCF values (stored from _compute_torques)
        # CCF groups: G2=L_Ankle+Toe (idx 2), G5=R_Ankle+Toe (idx 5)
        if not hasattr(self, '_last_ccf_group_mean') or self._last_ccf_group_mean is None:
            return torch.zeros(self.num_envs, device=self.device)

        # Need per-env CCF, not mean. Store it in _compute_torques.
        ...
        # Use tanh-like smooth reward: stance rewards positive CCF, swing rewards negative CCF
        l_reward = l_is_stance * torch.clamp(l_ankle_ccf, min=0) + (1 - l_is_stance) * torch.clamp(-l_ankle_ccf, min=0)
        r_reward = r_is_stance * torch.clamp(r_ankle_ccf, min=0) + (1 - r_is_stance) * torch.clamp(-r_ankle_ccf, min=0)
    ...
    def _compute_torques(self, actions):
        ...
        ccf_raw = actions[:, self._num_actions:]
        
        # Stage 1 Curriculum: Mask CCF to 0
        if self._vic_curriculum_stage == 1:
            ccf_raw = torch.zeros_like(ccf_raw)
        
        # 3. Expand actions to full DoF if necessary
        if self.humanoid_type in ["smpl", "smplh", "smplx"] and self.reduce_action:
            q_targets_full = torch.zeros([actions.shape[0], self._dof_size], device=self.device)
            q_targets_full[:, self.action_idx] = q_targets_raw
            ccf_full = torch.zeros([actions.shape[0], self._dof_size], device=self.device)
            ccf_full[:, self.action_idx] = ccf_raw
        else:
            q_targets_full = q_targets_raw
            # VIC: expand grouped CCF (N_groups dims) to full DOF (69 dims)
            if self._vic_ccf_num_groups > 0:
                ccf_full = torch.zeros([actions.shape[0], self._dof_size], device=self.device)
                for g_idx, dof_list in enumerate(self._ccf_group_dof_map):
                    ccf_full[:, dof_list] = ccf_raw[:, g_idx:g_idx+1]
            else:
                ccf_full = ccf_raw
        ...
        if flags.test and self._vic_curriculum_stage == 2:
            ...
            self._phase_ccf_log.append((phase[0].item(), ccf_raw[0].detach().cpu().numpy()))
