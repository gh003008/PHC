...
class HumanoidImVIC(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # VIC S16: Knee angle reward — track L_Knee + R_Knee DOF angles directly
        # (instead of body pose). 1D scalar joint target avoids the multiclip
        # rotation-tracking breakage seen in S15. Encodes natural swing-phase
        # flexion from the reference motion.
        self._knee_angle_reward_w = cfg["env"].get("knee_angle_reward_w", 0.0)
        self._knee_angle_reward_k = cfg["env"].get("knee_angle_reward_k", 20.0)
        ...
        if self._knee_angle_reward_w > 0:
            ...
            knee_dof_idx_list = [l_knee_body * 3 + a for a in range(3)] + \
