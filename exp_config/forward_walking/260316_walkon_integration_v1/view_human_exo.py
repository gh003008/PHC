...
def generate_smpl_xml(beta0=0.0):
    ...
    suffix = f"_b{beta0:.1f}" if beta0 != 0.0 else ""
...
for i, dof_name in enumerate(smpl_dof_names):
    ...
    if joint_name in SPINE_JOINTS:
        ...
        human_dof_props['stiffness'][i] = 200.0
        human_dof_props['damping'][i] = 20.0
...
for i, dof_name in enumerate(smpl_dof_names):
    if dof_name == "L_Hip_y":
        human_targets[i] = math.radians(3)
    elif dof_name == "R_Hip_y":
        human_targets[i] = math.radians(-3)
