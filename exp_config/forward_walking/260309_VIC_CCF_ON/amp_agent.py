...
class AMPAgent(common_agent.CommonAgent):
    ...
    def pre_epoch(self, epoch_num):
        ...
        if self.vec_env.env.task.humanoid_type in ["smpl", "smplh", "smplx"]:
            ...
            if hasattr(humanoid_env, '_reward_curriculum_switch_epoch') and humanoid_env._reward_curriculum_switch_epoch > 0:
                if epoch_num < humanoid_env._reward_curriculum_switch_epoch:
                    self._task_reward_w = humanoid_env._reward_w_stage1_task
                    self._disc_reward_w = humanoid_env._reward_w_stage1_disc
                else:
